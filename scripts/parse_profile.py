    ...
    for row in reader:
        ...
        if (message_name := get_value(row, MESSAGES_MESSAGE_NAME_COLUMN)) is not None:
            ...
            field = FieldProfile(
                ...
                field_type=get_value(row, MESSAGES_FIELD_TYPE_COLUMN),
                array=array_value,
                components=get_components(get_value(row, MESSAGES_COMPONENTS_COLUMN)),
                scale=get_scale(
                ...
                accumulate=get_accumulate(get_value(row, MESSAGES_ACCUMULATE_COLUMN)),
        ...
        for field_number, field in message.fields.items():
            for subfield in field.subfields:
                ...
                if isinstance(ref_field_name, str):
                    ...
                    for name, value in zip(ref_field_name, ref_field_value):
                        subfield.refs.append(find_subfield_ref(name, value, field_map))
...
def read_types() -> dict[str, Any]:
    ...
    for row in reader:
        if (type_name := get_value(row, TYPES_TYPE_NAME_COLUMN)) is not None:
            types[type_name] = FieldType(
                base_type=get_value(row, TYPES_BASE_TYPE_COLUMN), values={}
            )
            ...
            types[last_type_name].values[parsed_value] = FieldTypeValue(
                value_name=value_name, comment=comment
            )
